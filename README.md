...
1. fork and sync git
2. review script tags in HTML
3. explore the debugger
4. update the store name with "easley's technical books"
5. create function renderFooter(store) that renders the store info to the DOM
6. create function renderBook(book) that renders one book to the DOM
7. call renderBook(book) on all the books in data.js
